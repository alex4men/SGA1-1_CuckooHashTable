...
public class CuckooHashTable<Key> {
    ...
    public boolean insert(Key x) {
        ...
        while (true) {
            if (occupiedCellsCount[0] >= Math.floor(array[0].length * maxLoadFactor) ||
                    occupiedCellsCount[1] >= Math.floor(array[1].length * maxLoadFactor) || rehashesAttempts >= maxRehashesThrshld) {
                expand();
                insertAttempts = 0;
                rehashesAttempts = 0;
            }
