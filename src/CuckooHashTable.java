...
public class CuckooHashTable<K> {
    ...
    public CuckooHashTable(int tableSize) {
        array = (K[][]) new Object[numOfArrays][tableSize];
        currentSize = new int[numOfArrays];
        clearTable();
    }

    private Random r = new Random();

    private final int numOfArrays = 2;
    private final double maxLoadFactor = 0.50;
    private final int rehashThrshld = 1;
...
//    }
    ...
    public boolean insert(K x) {
        ...
        if (currentSize[0] >= array[0].length * maxLoadFactor || currentSize[1] >= array[1].length * maxLoadFactor)
            expand();

        if (array[0][hash(x, 0)] == null) {
            array[0][hash(x, 0)] = x;
            currentSize[0]++;
            System.out.println("Inserted in 1st array");
        } else if (array[1][hash(x, 1)] == null) {
            array[1][hash(x, 1)] = x;
            currentSize[1]++;
            System.out.println("Inserted in 2nd array");
        } else return false;
    ...
    // TODO: 20/09/16
    private void expand() {
//        rehash( (int) ( array.length / maxLoadFactor ) );
    }
//
//    private void rehash( )
//    {
